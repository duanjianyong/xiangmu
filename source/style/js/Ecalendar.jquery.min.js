$.fn.ECalendar = function(config) {
  ...
  if (!IsPC()) {
    if (config.type == 0) { _this.attr("type", "datetime-local"); } else { _this.attr("type", "date"); }
    if (callback) { _this.change(function() { callback(_this.val(), ECalendar); }) }
    return false;
  }
  ...
  _this.click(function(event) {
    ...
    if (height / 2 < top) { ECalendar.css({ "left": post.left + config.offset[0], "top": post.top - config.offset[1] - ECalendar.height() - 1, }); } else { ECalendar.css({ "left": post.left + config.offset[0], "top": post.top + _this.outerHeight() + config.offset[1], }); }
    _this.parent(".calendarWarp").css("zIndex", +10)
    ECalendar.show();
  });
  ECalendar.mousedown(function(event) { event.stopPropagation() });
  $(document).mousedown(function() {
    if (ECalendar.is(":visible")) { ECalendar.hide(0, function() {
        if (callback) callback(input.val(), ECalendar); }).parent(".calendarWarp").css("zIndex", 1); }
  });
  _this.on("change", function() {
    var date = _this.val() ? new Date(_this.val().replace(/-/g, "/")) : null;
    if (date == null || date == 'Invalid Date') { input.val("");
      _this.val(""); } else { setData(date); }
  });
  ...
  day.on("mousedown", "li", function(e) {
    ...
    drag = $(this);
    if (config.type == 0) { li.addClass('activ').css({ "overflow": "visible", "background": config.skin }).append('<div class="at12"><div class="pointer"></div></div>');
      ...
      if (drag[0].tagName == "LI") {
        if (Math.abs(my) > 1 || Math.abs(mx) > 1) {
          ...
          if (Math.abs(my) > 5 || Math.abs(mx) > 5) {
            ...
            ahours += 12;
          } else { drag.children('div').attr("class", "at12") }
          getHour = ahours;
          getMinutes = amin;
    ...
    if (drag) {
      if (drag[0].tagName == "LI") {
        ...
        ECalendar.hide(0, function() {
          if (callback) callback(input.val(), ECalendar); }).parent(".calendarWarp").css("zIndex", 1); } else {
        drag.children('i').remove();
        var m = drag.text();
        if (drag.hasClass("imonth")) setData(new Date(getFullYear, m - 1, getDate, getHour, getMinutes));
        if (drag.hasClass("iyear")) { setData(new Date(m, getMonth - 1, getDate, getHour, getMinutes)); }
        currenttime.animate({ opacity: 1 }, 600);
        if (callback) callback(input.val(), ECalendar);
      }
      drag = false;
  ...
  function init() { isinit = true;
    ...
    _this.after('<svg t="1493098552635" class="icon" style="position: absolute;right:' + (_this.height() - 20) / 2 + 'px;top:50%;margin-top: -10px;" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1006" xmlns:xlink="http://www.w3.org/1999/xlink" width="20" height="20"><defs><style type="text/css"></style></defs><path d="M856.576 200.192v54.784h58.368V814.08H109.568V256.512h56.832v-56.32H51.2V870.4h921.6V200.192h-116.224z" fill="#fffffc" p-id="1007"></path><path d="M819.2 102.4h-204.288v255.488L819.2 358.4V102.4z" fill="#EF4848" p-id="1008"></path><path d="M563.712 664.064h256V716.8h-256zM563.712 512h256v51.2h-256zM461.312 200.192h102.4v56.32h-102.4zM205.312 664.064h256V716.8h-256zM205.312 512h256v51.2h-256z" fill="#fffffc" p-id="1009"></path><path d="M410.112 102.4H204.8v256l205.312-0.512V102.4z" fill="#EF4848" p-id="1010"></path></svg>');
