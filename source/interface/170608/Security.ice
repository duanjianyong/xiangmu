...
module NetCore
{
	...
	sequence<string>  DevIDSeq;  //设备id数组
	sequence<["cpp:type:wstring"] string> wsSeq;     //设备id数组
    ...
    interface ObjMgt
    {
		...
		//jsonCont格式
		//[{"lng":value,"lat":value,[{"devid":value,"devname":value,"imsinum":value},...]},...]
	   ...
	   int UpdateSuspectTask              //更新疑似号码分析器（分析研判）
	    ...
	    int IMSIQuery                  //号码批量查询（分析研判）
		 ...
		 int BlackListFuzzyQuery            //黑名单模糊查询
		   (
			   ["cpp:type:wstring"]
			   string                       department,            // 部门 
			   ["cpp:type:wstring"]       
			   string                       matchword,             // 匹配字段    
			   int                          page,                  // 界面页数
			   int                          pagesize,              // 每页显示条数             
			   out int                      total,                 // 记录总条数      
               out BlackListSeq             Items                  // 黑名单序列
