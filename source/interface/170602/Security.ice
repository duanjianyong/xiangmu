...
module NetCore
{
	...
	struct AccompTask        //存储伴随号码任务信息
	{
		...
		int        startime;
		int        endtime;
		string     addrs;         //搜索的地址信息，内容形式为"(lng,lat),(lng,lat)..."
	}; 
    ...
    interface ObjMgt
    {
		...
		int GetHeatMapData                //获取热力图数据
			(
				...
				out string                   jsonCont               //获取的内容（json格式）
			);
	...
	{
		Principal* GetPrincipal(string name,string password) throws UserOwnInvalidCertificateException, UserHasNoPrincipalException;
	};
};
